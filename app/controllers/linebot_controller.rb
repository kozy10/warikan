class LinebotController < ApplicationController
      ...
      when '殿'
        reply_content(event, {
          ...
          template: {
            ...
            text: 'ご用件は？',
            actions: [
              { label: '割り勘に参加する', type: 'uri', uri: ENV["LINE_LIFF_USERS"] },
              { label: '支払いを登録する', type: 'uri', uri: ENV["LINE_LIFF_PAYMENTS"] },
  ...
  def check(event)
    ...
    User.where(room_id: room.room_id).each do |user|
      ...
      payments_text += "小計: #{payment_price_by_user.to_s(:delimited)}円\n\n"
    end
    total_price = Payment.where('room_id = ? and check_id = ?', room.room_id, room.check_id).sum(:price)
    price_per_person = total_price / room.number_of_members
